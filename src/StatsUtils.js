...
export const dailyStats = (tasks, groupBy, chartStyle, dataType, dateRange) => {
	...
	tasks = tasks.filter(
		(a) =>
			new Date(a.created_at) >= new Date(dateRange.startDate) &&
			new Date(a.created_at) <= new Date(dateRange.endDate),
	);
	var mapOfKeys = grouping(tasks, (task) => formatDate(task[groupBy.key]));
	var stats = calculateStat(mapOfKeys, groupBy, dataType);
	var dataArr = [];
	var labelArr = [];
	pushMapKeysToArray(mapOfKeys, labelArr);
	var options = {
		chart: {
			height: 1500,
			width: 900,
...
const calculateStatByCount = (map, chartTitleKey) => {
	...
	var statArr = [];
	pushMapKeysToArray(statOfKeys, statArr);
	var series = [
		{
...
const calculateStatByStatus = (map, chartTitleKey) => {
	...
	setOfStatus.forEach((a) => {
		var dataArr = [];
		pushMapValuesToArray(map, a, dataArr);
		var d = {
			name: `Tasks ${a}`,
			data: dataArr,
		};
...
function pushMapValuesToArray(map, a, dataArr) {
	try {
		if (map instanceof Map) {
			Array.from(map.keys()).forEach((b) => {
				var arr = map.get(b);
...
function pushMapKeysToArray(mapOfKeys, labelArr) {
	try {
		if (mapOfKeys instanceof Map) {
			Array.from(mapOfKeys.keys()).forEach((a) => {
				labelArr.push(a);
			});
		}
	} catch (e) {

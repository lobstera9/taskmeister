...
import { deleteDataById, getAllData, getById, updateData } from "./Dao.js";
...
const TaskView = () => {
	...
	var rowCss =
		"flex items-center space-x-4 mb-4 p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-all duration-300";
	...
	return (
		<>
			...
			<div className={rowCss}>
				<div className={labels}>Task Creation date:</div>{" "}
				{formatDateTime(task?.created_at)}
			</div>
			<div className={rowCss}>
				<div className={labels}>In Progress date:</div>
				{formatDateTime(task?.in_progress_at)}
			</div>
			<div className={rowCss}>
				<div className={labels}>Completion Date:</div>
			...
			<button className="bg-red-800 rounded-lg" onClick={handleDelete}>
				DELETE
			</button>
		</>
	);
